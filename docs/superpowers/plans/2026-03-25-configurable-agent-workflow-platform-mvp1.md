...
- Completed in branch `feat/mvp1-configurable-runtime`: Task 1 through Task 11.
- Delivered runtime modules: `spec_models`, `hook_registry`, `config_registry`, `agent_runtime`, `workflow_runtime`, `orchestrator`.
- Delivered API/runtime integration: `POST /chat` with optional `workflow_id`, `POST /admin/reload`, proposal default route switched to `proposal_v2` with rollback env.
- Delivered memory MVP-1: SQLite `raw_messages`, `working_context`, `compression_events`; working-context compression with full history retention.
- Delivered tooling additions: DOCX/PDF export tools and registry binding.
- Verification completed: backend full test suite pass (`70 passed`), startup smoke pass.
...
- [x] **Step 1: Write the failing tests (@superpowers:test-driven-development)**
...
- [x] **Step 1: Write failing whitelist test**
...
- [x] **Step 3: Implement whitelist resolver**
...
- [x] **Step 1: Write failing load/reload tests**
...
- [x] **Step 3: Implement registry + baseline YAMLs**
...
- [x] **Step 1: Write failing export tool tests**
...
- [x] **Step 3: Implement minimal export tools and register IDs**
...
- [x] **Step 1: Write failing agent runtime tests**
...
- [x] **Step 3: Implement agent runtime factory**
...
- [x] **Step 1: Write failing workflow routing/loop tests**
...
- [x] **Step 3: Implement workflow compiler/runtime with limits**
...
- [x] **Step 3: Implement orchestrator policy and state fields**
...
- [x] **Step 1: Write failing memory persistence/compression tests**
...
- [x] **Step 3: Update SQLite schema + memory node behavior**
...
- [x] **Step 3: Implement routes and service integration**
...
- [x] **Step 1: Write failing default-route test**
...
- [x] **Step 3: Implement replacement + rollback guard**
...
- [x] **Step 1: Write/adjust docs checklist test (manual assertion file)**
...
- [x] **Step 3: Smoke-run server startup**
...
- [x] **Step 4: Update docs with exact behavior**
...
## Post-MVP Follow-up TODO (Tracked)

- Epic B (Tools unification): unify `skills` / built-in `tools` / `mcp` under one abstraction with capability metadata and permission/cost/latency policy.
- Epic D (Runtime governance): add audit trails, observability dashboards, and config snapshot rollback/governance.
- Memory ops hardening: replace `datetime.utcnow()` usage with timezone-aware timestamps to remove runtime deprecation warnings.
- Dynamic-mode policy tuning: refine agent-selection heuristics and configurable clarification strategy.
