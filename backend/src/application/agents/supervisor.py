...
from src.domain.state import GlobalState, UserProfile, IntentClassification
from src.infrastructure.config.prompt import SUPERVISOR_PROMPT
...
def supervisor_node(state: GlobalState, llm: BaseLanguageModel) -> Dict:
    ...
    from src.application.agents.factory import build_supervisor
    ...
    # 将 Supervisor 提取出的主题写回全局状态，供 proposal/survey 工作流使用
    initial_topic = state.get("initial_topic")
    if intent.intent in ("PROPOSAL_GEN", "SURVEY_WRITE"):
        initial_topic = (intent.workflow_topic or latest_human or "").strip() or initial_topic
