class ResearchAgentUI {
    ...
    async connectWebSocket() {
        ...
        const wsUrl = `${wsProtocol}://${window.location.host}/ws/${this.sessionId}?access_key=${this.accessKey}`;
    ...
    updateAgentStep(data) {
        const agentNames = {
            ...
            'survey_writer': '✍️ Survey Writer'
        };
        
        const agentName = agentNames[data.node_name] || data.node_name;
        let message = `${agentName} is working...`;
        let details = '';
        const plan = data.plan || data.details;
        const critic = data.critic || data.critic_result;
        
        if (plan) {
            const actionType = plan.step_type || plan.action_type;
            if (actionType === 'search') {
                details = `Search query: ${plan.query || ''}`;
            } else if (actionType === 'synthesize') {
                details = 'Synthesizing research results...';
            }
            
            if (plan.has_enough_content !== undefined) {
                details += plan.has_enough_content ? ' (Content sufficient)' : ' (More content needed)';
            }
        }
    ...
    getStepDisplayName(step) {
        const stepNames = {
            ...
            'critic': 'Review',
            'reporter': 'Writing',
            'survey_researcher': 'Survey Research',
            'survey_writer': 'Survey Writing'
        };
...
${referencesContent}`;
        } else if (finalResult.success && finalResult.type === 'survey') {
            const sections = Array.isArray(finalResult.sections) ? finalResult.sections : [];
            const references = Array.isArray(finalResult.references) ? finalResult.references : [];

            const sectionsContent = sections.map((section) => (
                `## ${section.title || 'Untitled Section'}\n${section.content || ''}`
            )).join('\n\n<!-- -->\n\n');

            const referencesContent = references.length
                ? references.map((ref, index) => `${index + 1}. [${ref.title || 'Untitled'}](${ref.uri || '#'})`).join('\n')
                : 'No references available.';

            content = `# Literature Survey: ${finalResult.topic || 'Untitled Topic'}
...
${referencesContent}`;
        } else if (finalResult.success && finalResult.message) {
